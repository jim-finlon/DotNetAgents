...
namespace DotNetAgents.Core.Execution;
...
/// <summary>
/// Default implementation of <see cref="IExecutionContextProvider"/> using AsyncLocal.
/// </summary>
public class AsyncLocalExecutionContextProvider : IExecutionContextProvider
{
    private static readonly AsyncLocal<ExecutionContext?> _currentContext = new();

    /// <inheritdoc/>
    public ExecutionContext? GetCurrent() => _currentContext.Value;

    /// <inheritdoc/>
    public void SetCurrent(ExecutionContext? context) => _currentContext.Value = context;
}