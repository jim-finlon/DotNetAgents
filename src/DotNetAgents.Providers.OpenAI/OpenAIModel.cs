...
/// <summary>
/// OpenAI implementation of <see cref="ILLMModel{TInput, TOutput}"/> for chat completions.
/// </summary>
public class OpenAIModel : ILLMModel<string, string>
{
    private readonly HttpClient _httpClient;
    private readonly string _apiKey;
    private readonly string _modelName;
    private readonly ILogger<OpenAIModel>? _logger;
    ...
    /// <param name="modelName">The name of the model to use (e.g., "gpt-4", "gpt-3.5-turbo").</param>
    ...
    public OpenAIModel(
        ...
        ILogger<OpenAIModel>? logger = null)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger?.LogDebug("Calling OpenAI API with model {ModelName}", _modelName);
            ...
            {
                throw new AgentException("Failed to deserialize OpenAI response.", ErrorCategory.LLMError);
            }

            if (completionResponse.Choices == null || completionResponse.Choices.Length == 0)
            {
                throw new AgentException("OpenAI response contained no choices.", ErrorCategory.LLMError);
            }
            ...
            _logger?.LogDebug("OpenAI API call completed successfully. Tokens used: {Usage}", completionResponse.Usage?.TotalTokens ?? 0);
        ...
        catch (HttpRequestException ex)
        {
            ...
            throw new AgentException($"OpenAI API call failed: {ex.Message}", ErrorCategory.LLMError, ex);
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            _logger?.LogError(ex, "Timeout calling OpenAI API");
            throw new AgentException("OpenAI API call timed out.", ErrorCategory.LLMError, ex);
        }
        catch (JsonException ex)
        {
            _logger?.LogError(ex, "JSON deserialization error for OpenAI response");
            throw new AgentException("Failed to parse OpenAI response.", ErrorCategory.LLMError, ex);
        }
    }
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger?.LogError(ex, "HTTP error calling OpenAI streaming API");
            throw new AgentException($"OpenAI streaming API call failed: {ex.Message}", ErrorCategory.LLMError, ex);
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            _logger?.LogError(ex, "Timeout calling OpenAI streaming API");
            throw new AgentException("OpenAI streaming API call timed out.", ErrorCategory.LLMError, ex);
        }
    ...
    private string? ParseStreamChunk(string jsonData)
    {
        ...
        catch (JsonException ex)
        {
            ...
            // Return null to continue processing other chunks
        }
        return null;
    }
    ...
    private static int GetMaxTokensForModel(string modelName)
    {
        ...
        {
            var m when m.Contains("GPT-4", StringComparison.OrdinalIgnoreCase) => 8192,
            var m when m.Contains("GPT-3.5", StringComparison.OrdinalIgnoreCase) => 4096,
            _ => 4096 // Default
        };
    }
