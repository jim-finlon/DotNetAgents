...
/// <summary>
/// vLLM implementation of <see cref="ILLMModel{TInput, TOutput}"/> for local LLM chat completions.
/// vLLM provides an OpenAI-compatible API for high-performance local LLM inference.
/// </summary>
public class vLLMModel : ILLMModel<string, string>
{
    private readonly HttpClient _httpClient;
    private readonly string _modelName;
    private readonly ILogger<vLLMModel>? _logger;
    private readonly JsonSerializerOptions _jsonOptions;

    /// <summary>
    /// Initializes a new instance of the <see cref="vLLMModel"/> class.
    /// </summary>
        ...
        ILogger<vLLMModel>? logger = null)
    {
        ...
        // Configure HTTP client for vLLM API (OpenAI-compatible)
    ...
    /// <inheritdoc/>
    public int MaxTokens => 8192; // vLLM models typically support up to 8192 tokens
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger?.LogDebug("Calling vLLM API with model {ModelName}", _modelName);
            ...
            {
                throw new AgentException("Failed to deserialize vLLM response.", ErrorCategory.LLMError);
            }

            if (completionResponse.Choices == null || completionResponse.Choices.Length == 0)
            {
                throw new AgentException("vLLM response contained no choices.", ErrorCategory.LLMError);
            }

            var content = completionResponse.Choices[0].Message?.Content;
            if (content == null)
            {
                throw new AgentException("vLLM response contained no content.", ErrorCategory.LLMError);
            }

            _logger?.LogDebug("vLLM API call completed successfully");

            return content;
        }
        catch (HttpRequestException ex)
        {
            _logger?.LogError(ex, "HTTP error calling vLLM API");
            throw new AgentException($"vLLM API call failed: {ex.Message}", ErrorCategory.LLMError, ex);
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            _logger?.LogError(ex, "Timeout calling vLLM API");
            throw new AgentException("vLLM API call timed out.", ErrorCategory.LLMError, ex);
        }
        catch (JsonException ex)
        {
            _logger?.LogError(ex, "JSON deserialization error for vLLM response");
            throw new AgentException("Failed to parse vLLM response.", ErrorCategory.LLMError, ex);
        }
    }
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger?.LogError(ex, "HTTP error calling vLLM streaming API");
            throw new AgentException($"vLLM streaming API call failed: {ex.Message}", ErrorCategory.LLMError, ex);
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            _logger?.LogError(ex, "Timeout calling vLLM streaming API");
            throw new AgentException("vLLM streaming API call timed out.", ErrorCategory.LLMError, ex);
        }
    ...
    // Request/Response models for vLLM API (OpenAI-compatible)
